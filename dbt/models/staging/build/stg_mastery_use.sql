...
select
    date,
    value
from {{ source('external','ext_mastery_use') }}
),
...
-- Retain index for order & running total calculation in next step
stage_3 as (
select
...
-- Create running total of list elements to get player_id (foreign key)
select
    date,
    key,
